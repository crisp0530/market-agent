...
.PHONY: help config config-upgrade check install install-market-analyst dev dev-daemon start stop up down clean docker-init docker-start docker-stop docker-logs docker-logs-frontend docker-logs-gateway
...
# Install market-analyst dependencies
...
ifeq ($(OS),Windows_NT)
	...
	cd market-analyst && python3 -m venv .venv && .venv/bin/pip install -e ".[dev]"
endif
